...
def analyze_code(parsed_diff: List[Dict[str, Any]], pr_details: PRDetails) -> List[Dict[str, Any]]:
    ...
    for idx, file_data in enumerate(parsed_diff):
        file_path = file_data.get('path', '')
        print(f"\nProcessing file {idx + 1}/{len(parsed_diff)}: {file_path}")
        ...
        # Add 5 second delay after each file (except the last one)
        if idx < len(parsed_diff) - 1:
            time.sleep(5)
